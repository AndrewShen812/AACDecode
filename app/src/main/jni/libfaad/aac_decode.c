                         ...
                         float *song_length)
{
     ...
     * 以TAG三个字符开头，后面跟上歌曲信息。 v2版一般位于mp3的开头(以ID3字符开头)，
     * 可以存储歌词，该专辑的图片等大容量的信息。
    ...
    *song_length = length;
    ...
    do
    {
        ...
        if (first_time && !frameInfo.error)
        {
            ...
            LOGD("FrameInfo-->Oject Type=%s", tempstr);
            memset(tempstr,0,100);
            //Header Type
            ...
            LOGD("FrameInfo--Header Type=%s", tempstr);
            memset(tempstr,0,100);
            ...
            LOGD("FrameInfo--SBR(?HE-AAC(AAC+SBR)?)=%s", tempstr);
            memset(tempstr,0,100);
            //PS
            ...
            LOGD("FrameInfo--PS(?HE-AAC v2(AAC+SBR+PS)?)=%s", tempstr);
            free(tempstr);
            //-------------
            if (!adts_out)
            {
